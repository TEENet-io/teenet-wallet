...
| **Passkey session** | A bearer token prefixed `ps_` issued by the wallet after a successful WebAuthn login (hardware key or platform authenticator). Generated locally as a random token, stored in-memory with a 24-hour TTL, and paired with a CSRF token. Can do everything, including approving/rejecting pending requests and irreversible operations like wallet deletion. |
...
| **Mock service** | A standalone Go binary (`teenet-sdk/mock-server`) that implements the same HTTP API as the production TEENet service, using real cryptographic operations but with deterministic keys. Used for local development. |
