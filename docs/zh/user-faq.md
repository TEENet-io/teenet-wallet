...
OpenClaw 会引导你完成设置，你需要提供钱包 API 地址和 API 密钥。详细步骤请参考“快速上手”中的第三步。

### AI Agent（例如 OpenClaw）不需要我审批就能做什么？

在你设定的规则范围内，AI Agent（例如 OpenClaw）可以自主完成：
- 发送低于 USD 阈值的转账
- 查询任意钱包的余额和交易状态
- 读取合约数据（查余额、查价格等只读操作）
- 查看钱包地址和公钥信息

阈值越高，Agent 的自主权越大；阈值越低，你需要审批的频率越高。

### 什么操作一定需要我审批？

以下操作无论如何都需要你用 Passkey 确认：
- 超过 USD 阈值的任何转账
- 超出日限额的转账会被直接拒绝，不能通过审批放行
...
这些操作涉及资产安全或权限变更，系统设计上不允许任何 Agent 绕过你的确认。

### 可以用多个 Agent 吗？

可以。为每个 Agent 实例生成一个独立的 API 密钥即可。它们共享同样的审批策略和白名单。你可以在历史记录中区分不同实例的操作，也可以单独撤销某个实例的密钥。建议为每个实例使用不同的标签名（比如“openclaw-交易”和“openclaw-理财”），方便管理和追踪。

### Agent 花超了怎么办？

多重机制保护你的资产：
- **USD 阈值** -- 超过阈值的交易自动暂停等待你审批，Agent 无法绕过
- **日限额** -- 超过当日上限的转账会被直接拒绝，不能通过审批继续执行
- **白名单** -- Agent 只能与你授权的合约交互，无法将资金发送到未知合约
- **撤销密钥** -- 发现异常时，立即在 Web 界面撤销 API 密钥，Agent 的所有权限瞬间归零

### 怎么停止 Agent 使用我的钱包？

登录 [TEENet Wallet][wallet-url]，打开**设置**，在 API 密钥列表中找到对应的密钥，点击 **Revoke**（撤销）。密钥立即失效，Agent 将无法执行任何钱包操作。如果你不确定哪个密钥对应哪个 Agent 实例，可以撤销所有密钥，然后为需要保留的实例重新生成。
...
TEENet Wallet 支持区块链常见的主要签名方案。标记 **✓** 的链已做过端到端验证。
...
任何 EVM 兼容链都可以在运行时添加。让 Agent 列出可用链，或直接查看 API 中的 `GET /api/chains`。
