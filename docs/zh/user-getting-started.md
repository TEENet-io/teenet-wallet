...
3. 在邮箱中查收 6 位验证码，填回钱包页面。

4. 点击 **使用 Passkey 注册**。浏览器会提示你设置 **Passkey**：
...
3. 当 OpenClaw 提示时，输入：
   - **TEENET_WALLET_API_URL**：账户页中显示的钱包 API URL
   - **TEENET_WALLET_API_KEY**：第二步复制的 API 密钥

<div align="center"><img src="picture/tg.png" alt="通过 OpenClaw 连接钱包" width="360" /></div>

## 第四步：测试钱包

让 Agent 做一次快速测试，例如：*"帮我做一次钱包引导测试。"* 如果你还没有测试网钱包，Agent 会先帮你创建一个，然后引导你完成：
...
测试网 faucet：Sepolia ETH、Base Sepolia ETH、Solana Devnet SOL 都已内置在 Agent 中（直接让 Agent 领取即可）。外部 faucet 备用：[Sepolia ETH](https://faucet.google.com/ethereum/sepolia) · [Test USDC](https://faucet.circle.com)
