...
| `ALPHA_MODE` | `false` | When `true`, drops every chain whose `testnet` field is not `true` from the active registry at startup. The public alpha deployment sets this to `true` so only testnet chains are exposed via `/api/chains` and accepted by wallet creation. `chains.json` itself is untouched — it stays the full capability list. Self-hosted and custom deployments can leave this unset (or `false`) to expose every chain in `chains.json`. |
