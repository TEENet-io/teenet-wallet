...
| Linux (RHEL/Fedora/Rocky/AlmaLinux/阿里云 Linux) | 用 `dnf` 安装 `sqlite-devel` |
...
```bash
PASSKEY_RP_ORIGIN=http://localhost:<钱包端口> make run
```

浏览器 Passkey 注册要求 `scheme://host:port` 精确匹配,任何不一致都会直接报 origin 不匹配。
...
如果更改了 mock 端口,启动钱包时需要相应更新 `SERVICE_URL`。
